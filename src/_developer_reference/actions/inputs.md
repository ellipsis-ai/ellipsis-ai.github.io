...
Adding inputs to an [action]({% link _developer_reference/actions.md %}) allows you to collect answers from the user that can then be used in your [function]({% link _developer_reference/actions/function.md %}). Answers are collected after a user triggers the action, before the function runs.
...
The answers for each input can be collected from the [trigger text]({% link _developer_reference/actions/triggers.md %}) that triggered the action in the first place, if the trigger includes ["fill-in-the-blank" inputs]({% link _developer_reference/actions/triggers.md %}#fill-in-the-blank-input), or [regular expression capturing parentheses]({% link _developer_reference/actions/triggers.md %}#collecting-inputs-with-regular-expression-patterns).
